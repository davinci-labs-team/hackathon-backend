...
export class ConfigurationService {
  ...
  async create(newConfigurationData: CreateConfigurationDTO, supabaseUserId: string): Promise<ConfigurationResponse> {
  ...
  async update(key: string, updateConfigurationData: UpdateConfigurationDTO, supabaseUserId: string): Promise<ConfigurationResponse> {
    await this.validateUserRole(supabaseUserId);

    const config = await this.prisma.hackathonConfig.findUnique({ where: { key } });
